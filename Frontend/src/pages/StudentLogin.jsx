...
const StudentLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (tab === 'login') {
        await handleLogin(username, password, '/student', 'student');
      } else {
  ...
  return (
    <div className='relative flex min-h-screen items-center justify-center overflow-hidden bg-surface-950 px-4 py-12'>
      ...
      <div className='pointer-events-none absolute inset-0'>
        <div className='absolute -left-48 -top-48 h-[500px] w-[500px] rounded-full bg-emerald-500/8 blur-3xl' />
      ...
      <div className='relative w-full max-w-md animate-fade-up'>
        ...
        <div className='glass-card p-8'>
          ...
          <div className='mb-6 flex items-center gap-3'>
            <div className='flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-500/20 text-sm font-bold text-emerald-300'>
              S
            </div>
            <div>
              <p className='text-xs uppercase tracking-[0.18em] text-slate-400'>Student Portal</p>
              <h1 className='font-display text-xl text-white'>
                {tab === 'login' ? 'Welcome back, Learner' : 'Create your account'}
              </h1>
            </div>
          </div>
          ...
          <div className='mt-6 border-t border-white/10 pt-5 text-center'>
            <p className='text-sm text-slate-400'>
              Are you a teacher?{' '}
              <Link to='/login/teacher' className='font-semibold text-brand-300 hover:text-brand-200'>
                Teacher portal →
              </Link>
            </p>
          </div>
